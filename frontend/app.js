...
const interfaceToggle =
    document.getElementById("interfaceToggle");

const documentSupportButton =
    document.getElementById("documentSupportButton");

const interfaceSetting =
    document.getElementById("interfaceSetting");

const documentSetting =
    document.getElementById("documentSetting");

const STORAGE_FOCUS_MODE =
    "nexus_focus_mode";
...
function updateFocusModeUI() {
    ...
    if (interfaceToggle) {
        ...
        interfaceToggle.setAttribute(
            "aria-pressed",
            String(focusModeEnabled)
        );

        interfaceToggle.classList.toggle(
            "active",
            focusModeEnabled
        );
    }
...
function setFocusMode(enabled, showMessage = false) {
...
function loadFocusMode() {
    ...
    try {
        ...
        console.warn(
            "Focus mode load error:",
            error
        );
    }

    focusModeEnabled = saved;
    updateFocusModeUI();
}

interfaceToggle?.addEventListener(
    "click",
    event => {
...
interfaceSetting?.addEventListener(
    "click",
    event => {
        if (
...
documentSupportButton?.addEventListener(
    "click",
    event => {
        event.stopPropagation();

        // Return to the research workspace and open
        // the existing native file selector.
        showPage("home");
...
documentSetting?.addEventListener(
    "click",
    event => {
        if (
            event.target.closest(
