...
def web_search_crag(query, max_results=CRAG_WEB_RESULTS):
    """
    Corrective web retrieval with no API key and no extra package.

    Search order:
      1. Wikipedia public search API (stable JSON response)
      2. DuckDuckGo HTML
      3. Bing HTML fallback

    The first source is useful for general factual questions while the
    other engines provide broader web coverage.
    """
    found = []
    seen_urls = set()

    def add_result(title, url, snippet):
        title = re.sub(r"\s+", " ", unescape(str(title))).strip()
        url = unescape(str(url)).strip()
        snippet = re.sub(r"\s+", " ", unescape(str(snippet))).strip()

        if not url or not title or url in seen_urls:
            return

        # Skip obvious non-web/internal links.
    ...
    try:
        ...
        for item in wiki_data.get("query", {}).get("search", []):
            ...
            add_result(title, page_url, snippet)

            if len(found) >= max_results:
                return found[:max_results]

    except Exception as exc:
        print("CRAG Wikipedia search failed:", exc)
    ...
    try:
        ...
        # Handle both result__a/result__snippet and normal result blocks.
        pattern = re.compile(
        ...
        print("CRAG DuckDuckGo search failed:", exc)
    ...
    try:
        ...
        for match in pattern.finditer(html):
            ...
            snippet = re.sub(r"<.*?>", "", match.group(3) or "")
        ...
        print("CRAG Bing search failed:", exc)
